...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/runtime/skyline/share-element.html"
source_route: "/miniprogram/dev/framework/runtime/skyline/share-element.html"
raw_html: "raw-html/runtime/skyline/share-element.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/runtime/skyline/share-element.html](https://developers.weixin.qq.com/miniprogram/dev/framework/runtime/skyline/share-element.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/runtime/skyline/share-element.html`
...
![](../../assets/dd46af044201a570.png)

## 使用方法

| 属性 | 类型 | 默认值 | 必填 | 说明 | 最低版本 |
| --- | --- | --- | --- | --- | --- |
| key | string |  | 是 | 映射标记，页面内唯一 | [2.29.2](../../compatibility) |
| transition-on-gesture | boolean | false | 否 | 手势返回时是否进行动画 | [2.29.2](../../compatibility) |
| shuttle-on-push | string | `to` | 否 | 指定 `push` 阶段的飞跃物 | [2.30.2](../../compatibility) |
| shuttle-on-pop | string | `to` | 否 | 指定 `pop` 阶段的飞跃物 | [2.30.2](../../compatibility) |
| rect-tween-type | string | `materialRectArc` | 否 | 动画插值曲线 | [2.30.2](../../compatibility) |
| on-frame | worklet callback |  | 否 | 动画帧回调 | [2.30.2](../../compatibility) |
...
如果是通过 `setData` 设置的，可能会错过首帧。针对这种情况，可以 [使用 Component 构造器构造下一个页面](https://developers.weixin.qq.com/miniprogram/dev/reference/api/Component)，只要在组件 `attached` 生命周期前（含）通过 `setData` 设置上去，就会在首帧渲染。
...
![](../../assets/afba6b941dbb427a.gif) ![](../../assets/43eb84b0b2fb0367.gif)
...
**默认计算方式，以 `share-element` 节点占据空间确定始末位置，会产生跳变。** ![](../../assets/e37e22c84080ed71.png)

**以图片实际占据空间确定始末位置，始终采用单一 `mode` 效果，不会产生跳变。** ![](../../assets/485c759935aeca25.png)

使用最新 `nightly` 工具预览，移动端安卓 8.0.33 版本。

[示例代码片段-进阶](https://developers.weixin.qq.com/s/vZ8Ydymg7ZMs)

![](../../assets/c3144af5016cefe9.gif)
