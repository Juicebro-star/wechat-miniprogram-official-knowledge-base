...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/runtime/operating-mechanism.html"
source_route: "/miniprogram/dev/framework/runtime/operating-mechanism.html"
raw_html: "raw-html/runtime/operating-mechanism.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/runtime/operating-mechanism.html](https://developers.weixin.qq.com/miniprogram/dev/framework/runtime/operating-mechanism.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/runtime/operating-mechanism.html`
...
![小程序生命周期](../assets/81cb8d233c74ae1a.svg)
...
从小程序生命周期的角度来看，一般讲的「**启动**」专指冷启动，热启动一般被称为后台切前台。
...
当使用了[后台音乐播放](https://developers.weixin.qq.com/miniprogram/dev/api/media/background-audio/wx.getBackgroundAudioManager.html)、[后台地理位置](https://developers.weixin.qq.com/miniprogram/dev/api/location/wx.startLocationUpdateBackground.html)等能力时，小程序可以在「后台」持续运行，不会进入到「挂起」状态
...
> 基础库 1.1.0 及以上，1.4.0 以下版本: 当用户从扫一扫、转发等入口（[场景值](../app-service/scene)为1007, 1008, 1011, 1025）进入小程序，且没有置顶小程序的情况下退出，小程序会被销毁。

## 小程序启动时打开的页面

小程序冷启动时，打开的页面有以下情况

- （A 类场景）若启动的场景中不带 path，则遵循 [重新启动策略](#重新启动策略) ，可能是首页或上次退出的页面
- （B 类场景）若启动的场景中带有 path，则启动进入对应 path 的页面

小程序热启动时，打开的页面有以下情况

- （A 类场景）若启动的场景中不带 path，则保留上次的浏览的状态
- （B 类场景）若启动的场景中带有 path，则 autoReLaunch 自动重加载到对应 path 的页面

A 类场景常见的有下列[场景值](../app-service/scene)：
...
在页面对应的 json 文件中（也可以全局配置在 app.json 的 window 段中），指定 `restartStrategy` 配置项可以改变这个默认的行为，使得从某个页面退出后，下次 A 类场景的冷启动可以回到这个页面。
...
> 基础库 2.7.4 开始支持，低版本需做[兼容处理](../compatibility)。
...
一个更完整的示例：[在开发者工具中预览效果](https://developers.weixin.qq.com/s/ELP5uTmN7E8l "在开发者工具中预览效果")
