...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/release/"
source_route: "/miniprogram/dev/framework/release/"
raw_html: "raw-html/release/index.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/release/](https://developers.weixin.qq.com/miniprogram/dev/framework/release/)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/release/index.html`
...
1. `A` 新增 API 跳转信贷代扣接口 wx.requestAutomaticRepayment [详情](https://developers.weixin.qq.com/miniprogram/dev/api/navigate/wx.requestAutomaticRepayment.html)
2. `A` 新增 API wx.saveMediaToPhotosAlbum 混合保存图片视频 [详情](https://developers.weixin.qq.com/miniprogram/dev/api/media/video/wx.saveMediaToPhotosAlbum.html)
3. `U` 更新 组件 贴图组件和 api 优先展示小程序简称，若无简称则使用小程序名称
4. `U` 更新 组件 keyboard-accessory 支持鸿蒙 [详情](https://developers.weixin.qq.com/miniprogram/dev/component/keyboard-accessory.html)
5. `F` 修复 框架 小游戏 PC 端订阅消息「总是保持以上选择」勾选不生效

## v3.17.1 (2026-07-01)

1. `F` 修复 框架 glass-easel webview TabBar 创建时序导致的错误
2. `F` 修复 框架 skyline 模式下 wx:for 内的 placeholder 不更新问题
3. `F` 修复 组件 scroll-view 首次加载时异常高度 [详情](https://developers.weixin.qq.com/community/develop/doc/000ca639a60af0c1b3155710e6bc00)
4. `F` 修复 插件中使用 httpDNS 的问题
...
3. `U` 更新 框架 聊天工具开放能力支持参数控制单选 [详情](../open-ability/chatToolOpenMode)
...
1. `U` 更新 框架 httpdns 支持插件发起 [详情](../ability/HTTPDNS)
...
4. `U` 更新 框架 优化手机号扣费
5. `U` 更新 框架 聊天工具开放能力 [详情](../open-ability/chatTool)
...
1. `A` 新增 框架 小程序用工关系功能相关接口 [详情](../open-ability/laboruse/intro)
...
2. `A` 新增 API 支持路由重写 wx.rewriteRoute [详情](../app-service/route-rewrite)
3. `U` 更新 框架 skyline 支持 @keyframes 规则可选择全局共享或按 styleIsolation 隔离 [详情](../runtime/skyline/wxss)
4. `U` 更新 框架 小程序启动性能优化
...
1. `A` 新增 框架 支持小程序封面广告展示状态获取 [详情](wx.getShowSplashAdStatus )
...
1. `U` 更新 框架 性能诊断工具增加启动时序图 [详情](../performance/perf_diagnostic_tool)
...
4. `U` 更新 框架 渲染缓存新增 capture 模式 [详情](../view/initial-rendering-cache)
...
1. `A` 新增 框架 基础库 HarmonyOS 适配 [详情](../ability/ohos)
2. `A` 新增 框架 小程序性能检测工具 [详情](../performance/perf_diagnostic_tool)
...
3. `U` 更新 框架 glass-easel 组件框架支持 WXS 事件响应 [详情](../custom-component/glass-easel/migration)
...
1. `A` 新增 框架 一组路由事件监听 [详情](../app-service/route-event-listener)
...
3. `U` 更新 框架 支持 Chaining API init 写法绑定 worklet 回调 [详情](../runtime/skyline/gesture)
...
2. `U` 更新 API 有授权弹窗的隐私接口（例如 wx.getLocation），将不会再弹官方隐私弹窗，而是在授权弹窗上增加“隐私勾选”；无授权弹窗的隐私接口（如 wx.getClipboardData），将继续保留原来的官方隐私弹窗 [详情](../user-privacy/PrivacyAuthorize#%E5%85%AD%E3%80%81%E5%AE%98%E6%96%B9%E9%9A%90%E7%A7%81%E5%BC%B9%E7%AA%97%E5%8A%9F%E8%83%BD%E8%AF%B4%E6%98%8E)
...
4. `U` 更新 框架 授权弹窗上耦合隐私勾选 [详情](../user-privacy/PrivacyAuthorize#%E5%85%AD%E3%80%81%E5%AE%98%E6%96%B9%E9%9A%90%E7%A7%81%E5%BC%B9%E7%AA%97%E5%8A%9F%E8%83%BD%E8%AF%B4%E6%98%8E)
...
3. `U` 更新 框架 页面返回手势封装 [详情](../runtime/skyline/pop-gesture)
...
2. `U` 更新 框架 XR-FRAME 支持3D body、3D hand、3D Face、试鞋能力 [详情](/miniprogram/dev/component/xr-frame/)
...
1. `A` 新增 组件 支持全局跨页面组件 [详情](../runtime/skyline/appbar)
...
2. `U` 更新 框架 visionKit 支持身份证识别能力 [详情](../open-ability/visionkit/idcard)
...
1. `A` 新增 框架 visionKit 支持试鞋能力 [详情](../open-ability/visionkit/shoe)
...
1. `U` 更新 框架 粒子系统需要支持子发射器 [详情](/miniprogram/dev/component/xr-frame/)
...
3. `U` 更新 框架 XR-FRAME ShareSystem 新增了录屏功能，受限于设备性能，录屏时请选择适当的录制帧率和分辨率 [详情](/miniprogram/dev/component/xr-frame/)
...
5. `U` 更新 框架 基础库预设自定义路由动画 [详情](../runtime/skyline/preset-route)
...
5. `U` 更新 框架 XR-FRAME 可视化TOOLKIT发布，取代CLI，支持环境数据生成，glTF优化，新增压缩纹理功能 [详情](/miniprogram/dev/component/xr-frame/)
6. `U` 更新 框架 XR-FRAME 视频纹理支持pause/resume方法，暴露了播放状态`EVideoState`，增加配置`autoPause`，小程序压后台时自动暂停/恢复时自动播放 [详情](/miniprogram/dev/component/xr-frame/)
...
8. `U` 更新 框架 启动页无法绑定自定义路由 [详情](../runtime/skyline/custom-route#%E5%A4%9A%E7%B1%BB%E5%9E%8B%E8%B7%AF%E7%94%B1%E8%B7%B3%E8%BD%AC)
...
10. `U` 更新 框架 XR-FRAME VideoTexture 发布正式版 [详情](/miniprogram/dev/component/xr-frame/)
11. `U` 更新 框架 Skyline 渲染引擎下，组件框架切换为 glass-easel [详情](../custom-component/glass-easel/migration)
...
3. `U` 更新 框架 允许在 json 文件中使用 options 定义段来配置页面和自定义组件选项
...
3. `U` 更新 框架 自定义组件支持 mut-bind [详情](../component-framework/index)
...
26. `F` 修复 组件 [image](https://developers.weixin.qq.com/miniprogram/dev/api/base/performance/wx.preloadAssets.html) 安卓 // 开头的图片链接没有追加 http 前缀 [反馈详情](https://developers.weixin.qq.com/community/develop/doc/000e8aa6d50498c1b558ad10856800>)
...
1. `A` 新增 框架 自定义组件增强样式隔离支持 [详情](../observer)
...
1. `A` 新增 框架 自定义组件支持数据监听器 [详情](../observer)
...
1. `A` 新增 框架 支持页面级自定义导航配置 `navigationStyle: custom` [详情](../config#页面配置)
2. `A` 新增 框架 支持页面禁止右滑返回手势 `disableSwipeBack: true` [详情](../config#页面配置)
3. `A` 新增 框架 如需使用位置接口，需在 `app.json` 中添加配置 `permission` 字段用于说明位置接口用途 [详情](../config#全局配置)
