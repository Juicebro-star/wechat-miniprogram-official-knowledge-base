...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/usability/PublicErrno.html"
source_route: "/miniprogram/dev/framework/usability/PublicErrno.html"
raw_html: "raw-html/usability/PublicErrno.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/usability/PublicErrno.html](https://developers.weixin.qq.com/miniprogram/dev/framework/usability/PublicErrno.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/usability/PublicErrno.html`
...
| 112 | api scope is not declared in the privacy agreement | 接口未在隐私协议中声明 |
...
| 1024 | invalid plugin appid | 不合法的插件appid |
| 1025 | wxa appid privacy api banned | 小程序隐私接口被封禁，[解决方案参考链接](https://developers.weixin.qq.com/community/develop/doc/00062a6d514c88baacdf52e8a56009) |
| 1026 | wxgame appid privacy api banned | 小游戏隐私接口被封禁，[解决方案参考链接](https://developers.weixin.qq.com/community/minigame/doc/0004c84925817819b7ffd8b2356008) |
| 1032 | invalid signature | 签名无效 |
| 1033 | data size not match | 列表数据长度不匹配 |
| 1034 | same user | 是同一用户 |
| 1035 | not wechat user | 非微信用户 |
| 1036 | invalid username | 非法的username |
| 100001 | json parse error | json解析错误 |
| 101100 | interface is not declared in the privacy agreement | 接口未在隐私协议中声明 |
| 101101 | privacy interface is banned | 隐私接口已封禁 |
| 101102 | privacy permission is not authorized | 用户未同意授权隐私协议 |
...
| 600003 | network interrupted error | 网络被迫中断。以下2种情况会导致该错误：1. 当小程序处于后台时发起网络请求；2. 小程序在前台发起网络请求后，退到后台，同时请求未能在退后台后的 5s 内结束时。 |
...
| 605103 | WebSocket connect is fail | WebSocket connect失败 |
...
| 702001 | invalid parameter | 参数错误 |
| 702002 | invalid signature | 用户态签名错误 |
| 702003 | invalid pay signature | 支付签名错误 |
| 702004 | invalid mode | 支付模式不合法 |
| 702005 | out_trade_no repeated, please change | 商户订单号重复 |
| 702006 | mch is not registered | 商户未进件完成, 不能下单 |
| 702007 | user unauthorized | 用户未授权, 不能下单 |
| 702008 | can not use sandbox in release version | 线上版本小程序不能用沙箱环境下单 |
| 703001 | user disable globalpay | 该用户无法使用全球收银服务 |
| 703002 | user cancel picker | 选择面板：用户取消 |
| 703003 | already picked | 选择面板：用户已选择，无需重复调用 |
| 703004 | load picker timeout | 选择面板：打开超时 |
| 703005 | picker already open | 选择面板：已打开 |
| 703006 | user unpicked payment method | 用户未选择支付方式，请先调用 openMethodPicker |
| 703007 | requestGlobalPayment already end | 支付已结束 |
| 703008 | input info error | 传入的订单信息错误 |
| 703009 | input info is inconsistent with the user's selection | 传入的订单信息对应的支付方式与用户选择的不一致 |
| 703010 | open paypage error | 打开支付页面失败 |
| 703011 | call abort | 由于调用 abort 接口，中断支付流程。 |
| 703012 | request timeout | 超时未调用 requestGlobalPayment |
| 703013 | request end | 支付结束，但无法判断用户是否成功支付。 |
| 703014 | user unpicked payment method | 用户未选择支付方式，请先调用 openMethodPicker |
| 703015 | requestGlobalPayment already end | 支付已结束 |
| 703016 | user is paying | 用户正在支付页面中 |
...
| 1400001 | appid no enough quota | 小程序quota余额不足 |
| 1402101 | wxa login is blocked | 小程序登录被封禁 |
...
| 1416104 | empty finder info | 视频号获取到的数据为空 |
| 1416105 | get partially visible finder info | 视频号获取到的数据仅部分人可见 |
| 1416106 | invalid finder token | 非法的视频号token |
| 1416107 | appid token no permission | 视频号账号关闭小程序token引用 |
| 1416108 | parse token failed | 解析token失败 |
| 1416109 | user token hit spam limit | 用户解析token时命中风控限制 |
| 1420103 | not same contractor | 小程序appid和企业微信的同主体校验不通过 |
| 1421102 | user getPhoneNumber is blocked | 用户获取手机号能力被封禁 |
...
| 2004000 | model path invalid : %s | 非法的模型文件路径 |
...
| 2004007 | input tensor [%s] data is invalid : %s | input tensor数据为空 |
| 2004008 | input tensor has invalid shape : %s | 无效的shape |
...
| 2004012 | the input tensor [%s] MUST have %s attribution | 输入张量缺失属性 |
