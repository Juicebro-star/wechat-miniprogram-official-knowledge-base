...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/visionkit/shoe.html"
source_route: "/miniprogram/dev/framework/open-ability/visionkit/shoe.html"
raw_html: "raw-html/open-ability/visionkit/shoe.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/visionkit/shoe.html](https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/visionkit/shoe.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/open-ability/visionkit/shoe.html`
...
首先需要创建 [VKSession](base) 的配置，然后通过 `VKSession.start` 启动 VKSession 实例。
...
需要保证在 [VKSession.start 接口](base) 调用，开启 VKSession 后。
调用 `VKSession` 的 `updateMaskMode` 更新是否开启腿部分割纹理的返回。然后通过 `VKFrame` 的 `getLegSegmentBuffer` 获取具体的腿部遮挡纹理buffer，然后基于 buffer 创建纹理，进行对应的遮挡剔除。
...
表示在 鞋子矩阵 作用后，每个点的三维偏移位置。

以下为关键点图示与点位解释：

![](https://res8.wxqcloud.qq.com.cn/wxdoc/f81c7ce6-a67e-4209-acfa-a84b70d059c4.png)
...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/00b2dee8-7b67-4a3b-bf40-fcbb3b174c4a.png)
...
[](https://mmbizwxaminiprogram-1258344707.cos.ap-guangzhou.myqcloud.com/xr-frame/doc/ar/12.mp4)
