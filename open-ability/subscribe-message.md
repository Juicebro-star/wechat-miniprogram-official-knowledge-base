...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/subscribe-message.html"
source_route: "/miniprogram/dev/framework/open-ability/subscribe-message.html"
raw_html: "raw-html/open-ability/subscribe-message.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/subscribe-message.html](https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/subscribe-message.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/open-ability/subscribe-message.html`
...
![intro](https://res8.wxqcloud.qq.com.cn/wxdoc/88ea87c8-68d3-4995-abf4-e7d030402b9e.jpg)
...
一次性订阅消息、长期订阅消息，详见服务端接口 [subscribeMessage.send](https://developers.weixin.qq.com/miniprogram/dev/server/API/mp-message-management/subscribe-message/api_sendmessage)，次数限制：开通支付能力的小程序下发上限是3kw/日，没开通的是1kw/日。

设备订阅消息，详见服务端接口 [hardwareDevice.send](https://developers.weixin.qq.com/miniprogram/dev/server/API/hardware-device/api_sendhardwaredevicemessage)
...
**1、当用户触发订阅消息弹框后，用户的相关行为事件结果会推送至开发者所配置的服务器地址或[微信云托管](https://developers.weixin.qq.com/miniprogram/dev/wxcloudservice/wxcloudrun/src/guide/weixin/push)服务。**
...
**2、当用户在手机端服务通知里消息卡片右上角“...”管理消息时，相应的行为事件会推送至开发者所配置的服务器地址或[微信云托管](https://developers.weixin.qq.com/miniprogram/dev/wxcloudservice/wxcloudrun/src/guide/weixin/push)服务。（目前只推送取消订阅的事件，即对消息设置“拒收”）**
...
**3、调用订阅消息接口发送消息给用户的最终结果，会推送下发结果事件至开发者所配置的服务器地址或[微信云托管](https://developers.weixin.qq.com/miniprogram/dev/wxcloudservice/wxcloudrun/src/guide/weixin/push)服务。**
...
当用户开启了语音提醒，开发者通过[wx.getSetting](https://developers.weixin.qq.com/miniprogram/dev/api/open-api/subscribe-message/wx.requestSubscribeDeviceMessage.html)获取的该模板的订阅状态为'acceptWithAudio'。

订阅弹窗样式如下：

![intro](https://res8.wxqcloud.qq.com.cn/wxdoc/09d44ab9-547d-48d4-8f21-c5ff2066c40a.png)
...
1. 1.用户将服务通知设置为免打扰
2. 2.用户开启了手机静音模式或手机音量过低
3. 3.用户未打开微信新消息通知，可引导用户前往微信-“我”-“设置”-“新消息通知”中打开
4. 4.用户未打开系统对微信的通知
5. 5.用户开启了低电量模式
6. 6.用户版本过低：需要iOS 8.0.6与安卓8.0.3及以上

## 订阅消息添加提醒

从基础库 [2.22.0](../compatibility) 开始支持

用户在订阅小程序**长期订阅消息**时，可以根据自己的使用情况添加提醒。添加后，用户在收到消息时，在微信内将由横幅通知提醒。

当用户添加了提醒，开发者通过[wx.getSetting](https://developers.weixin.qq.com/miniprogram/dev/api/open-api/subscribe-message/wx.requestSubscribeDeviceMessage.html)获取的该模板的订阅状态为'acceptWithAlert'。

订阅弹窗样式如下：

![intro](https://res8.wxqcloud.qq.com.cn/wxdoc/c6451328-8d42-48f5-817d-5667e890f908.png)
