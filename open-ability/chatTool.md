...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/chatTool.html"
source_route: "/miniprogram/dev/framework/open-ability/chatTool.html"
raw_html: "raw-html/open-ability/chatTool.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/chatTool.html](https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/chatTool.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/open-ability/chatTool.html`
...
![](https://res.wx.qq.com/op_res/Q7aL8rFfZ8uTLdWnPkvkDsFu4P6cN8MH9P_qdZJCvdV0cy-Y1QNyLaYg9PlVhsY9bP0StbwoiZ4cussTGo0RuQ)

## 二、开发指南

从基础库 [3.7.8](../compatibility) 开始支持

支持平台：Android：微信8.0.56及以上版本、iOS：微信8.0.56及以上版本

### 1. 聊天工具分包配置

开发者需要在[小程序全局配置](https://developers.weixin.qq.com/miniprogram/dev/reference/configuration/app) (app.json)中声明 `subPackages` 与 `chatTools`：
...
| scopes | string[] | 否 | 分包中会使用的[scope](authorize)权限 |
...
1. 当小程序处于普通模式时，开发者调用 `wx.openChatTool` 但不传入聊天室id，微信会拉起聊天列表让用户选择，用户选择后绑定聊天室进入聊天工具模式
2. 当小程序处于普通模式时，开发者调用 `wx.openChatTool` 并在接口中传入聊天室id（群聊为opengid，单聊为open_single_roomid），会直接绑定该聊天室进入
...
开发者可调用[wx.selectGroupMembers](https://developers.weixin.qq.com/miniprogram/dev/api/chattool/wx.selectGroupMembers.html)让用户选择聊天室的成员，并返回选择成员的group_openid。需要注意的是，若当前聊天室为单聊，则直接返回对方用户的group_openid，不再拉起选择器。

#### 3.3 渲染聊天成员的头像昵称

开发者可通过 [`open-data-list`](https://developers.weixin.qq.com/miniprogram/dev/component/open-data-item.html) 与 [`open-data-item`](https://developers.weixin.qq.com/miniprogram/dev/component/open-data-item.html) 渲染聊天成员的头像昵称
...
3. `form bind:submitToGroup="onSubmitToGroup"` 与 `button form-type="submitToGroup"`：将输入框内的文本内容发送到绑定的聊天室
...
服务端通过[创建activity_id接口](https://developers.weixin.qq.com/miniprogram/dev/server/API/mp-message-management/updatable-message/api_createactivityid)创建activity_id
...
服务端通过 [setChatToolMsg](https://developers.weixin.qq.com/miniprogram/dev/server/API/mp-message-management/updatable-message/api_setchattoolmsg) 接口更新活动状态或用户完成情况。
