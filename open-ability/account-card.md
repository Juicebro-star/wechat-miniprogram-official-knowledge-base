...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/account-card.html"
source_route: "/miniprogram/dev/framework/open-ability/account-card.html"
raw_html: "raw-html/open-ability/account-card.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/account-card.html](https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/account-card.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/open-ability/account-card.html`
...
![](https://res.wx.qq.com/op_res/PwJCvNMxQor5vPPZudAqi-2zpHTk_FAgUIh98sJk2KMGCGxg7I4uhkX8K_lHKX86IpiuwMzNMNk-KKOEBz0zHQ)

## 二、接入指南

支持微信版本：8.0.41 for Android 及以上；8.0.46 for iOS 及以上
...
| 卡片 ID | 卡片名称 | 类目要求 | 使用场景 | 展示字段 |
| --- | --- | --- | --- | --- |
| 5001 | 基金账户 | 金融业-公募基金、私募基金 | 向用户展示资产、收益情况等信息 | 总资产、昨日收益 |
| 5002 | 电信账户 | IT 科技-基础电信运营商、电信业务代理商 | 向用户展示话费余额、剩余流量等信息 | 话费余额、流量余额 |
...
登录[微信公众平台](https://mp.weixin.qq.com/)，符合类目要求即可在「开发管理」-「其他接口」中找到「发现页账户卡片」，点击开通，即可开通该功能开发权限。

### 2. 在代码包中创建卡片分包

账户卡片为小程序代码包中的独立组件分包。为保证 UI 统一，微信侧会提供不同类型的账户卡片模板，页面样式（`WXML` 与 `WXSS`）是由微信提供的固定的代码，`JS` 逻辑由开发者控制，包括数据请求、渲染、缓存等。

开发者需使用[微信开发者工具开发版 Nightly Build 1.06.2309062](https://developers.weixin.qq.com/miniprogram/dev/devtools/log#nightly) 及以上版本进行创建。具体步骤如下：

#### 2.1 创建分包目录 accountCardPackage

![](https://res.wx.qq.com/op_res/1PUwEs3n4tV5Ttwr9sTPXJHc012XVF-4v8FSuhCle5BQW-Hvzz4RoApbXclOzMbHQzUmRmswM1NwYseldc-6Og)
...
2. 在 `accountCardPackage` 分包目录中右键，点击新建卡片，输入组件名后创建（图为示例，所有类型卡片都从该入口新建）

![](https://res.wx.qq.com/op_res/1PUwEs3n4tV5Ttwr9sTPXBaQF196Prwkxr89mhS3iVDuXCpTXme7mlJf5y84AFHafop_EHsnXmEMqWdKBxyksw)

3. 完成以上步骤后，可在 `accountCardPackage` 目录下看到创建好的卡片，同时 `app.json` 中也会出现相关配置

![](https://res.wx.qq.com/op_res/1PUwEs3n4tV5Ttwr9sTPXCoU3FnZktRZ21puYXo0XrV3QGr5b6bcRBlj_YgC76YU0PPYCrNeJJSsvrZxTaKIPw)
...
创建卡片分包后，开发者可在 `js` 文件中实现卡片组件逻辑，包括获取登录态、请求数据、加解密数据等。一个卡片组件逻辑示例如下：
...
1. 设置卡片展示的变量信息。由于 `WXML` 文件不支持开发者自定义，因此每种卡片支持显示的字段由平台定义，开发者实现获取与加密逻辑后通过 `setData` 变更变量信息。不同卡片类型支持的变量信息可参考下表：

| 卡片 ID | 卡片名称 | 字段 | 描述 | 是否可隐藏 |
| --- | --- | --- | --- | --- |
...
|  |  | cardNumber | 卡片尾号，仅展示后四位，默认不显示 | 是 |
|  |  | insuranceAccountCnt | （多卡情况）共 x 个保险，默认不展示 | 是 |
...
|  |  | securitiesCnt | （多证券情况）全部 x 个证券账户，默认不展示 | 是 |
|  |  | allSecuritiesEnterPath | （多卡情况）若调用了 `securitiesCnt`，则需设置多卡显示区域设置对应的跳转路径 |  |

2. 更新卡片状态。为了满足不同情况下的展示诉求，卡片自身是存在状态的，开发者根据需求可以通过组件的 `updateStatus` 接口来更新状态。目前支持的状态与变更状态的方法如下：
...
3. 更新卡片点击跳转路径。卡片被点击时，默认是跳转至小程序首页，如果需要跳转到其他路径，可以通过组件的 `updateEnterPath` 接口来调整点击跳转路径：

```
this.updateEnterPath('pages/other/index')
```

4. 隐藏/显示部分字段。由于业务原因，部分字段无法展示，开发者可以隐藏/显示该字段的展示，具体方法为：
...
其中，需要注意的是，如 5003/5004 卡片有可隐藏日期的能力，需要隐藏整个日期字段：

```
this.hideField('dueDate')
```

5. 开发业务逻辑。目前在卡片组件内仅支持以下 `wx` 接口，开发者可通过以下接口实现业务逻辑：
...
![](https://res.wx.qq.com/op_res/1PUwEs3n4tV5Ttwr9sTPXOAPb3BUxBooRnI__jh5ZrkGqetFZvZWa1OZ7aPiRHVcy34KaOroS8CCuopyP0RWmw)

2. 只有体验版卡片支持输出内容到调试面板，若列表中没有体验版卡片，调试面板便无法打开。开发者可调用卡片组件的 `debugLog` 接口输出内容到调试面板中：
...
1. 卡片准备完成后，需要通过 `wx.addAccountCard` 接口让用户主动触发添加卡片：
...
- 用户当前微信版本不支持此功能（需为 8.0.41 for Android 及以上或 8.0.46 for iOS 及以上）

2. 开发者可以提前通过 `wx.checkAccountCardAddState` 检测卡片是否可添加、是否已添加：
