...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/user-privacy/PrivacyAuthorize.html"
source_route: "/miniprogram/dev/framework/user-privacy/PrivacyAuthorize.html"
raw_html: "raw-html/user-privacy/PrivacyAuthorize.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/user-privacy/PrivacyAuthorize.html](https://developers.weixin.qq.com/miniprogram/dev/framework/user-privacy/PrivacyAuthorize.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/user-privacy/PrivacyAuthorize.html`
...
2）新增官方隐私授权弹窗功能，相关功能参考下方[官方隐私弹窗功能说明](#六、官方隐私弹窗功能说明)。

## 二、接入流程

### 1. 配置《小程序用户隐私保护指引》

开发者需在「小程序管理后台」配置《小程序用户隐私保护指引》，详细指引可见：[用户隐私保护指引填写说明](index)。

需要注意的是，仅有在指引中声明所处理的用户信息，才可以调用平台提供的对应接口或组件。若未声明，对应接口或组件将直接禁用。隐私接口与对应的处理的信息关系可见：[小程序用户隐私保护指引内容介绍](miniprogram-intro)。
...
从基础库 [2.32.3](../compatibility) 开始支持
...
从基础库 [2.32.3](../compatibility) 版本起，[隐私同意按钮](https://developers.weixin.qq.com/miniprogram/dev/api/open-api/user-info/wx.getUserProfile.html)支持与[手机号快速验证组件](../open-ability/getPhoneNumber)、[手机号实时验证组件](../open-ability/getRealtimePhoneNumber)耦合使用，调用方式为 `<button open-type="getPhoneNumber|agreePrivacyAuthorization">` 或 `<button open-type="getRealtimePhoneNumber|agreePrivacyAuthorization">`。

也支持[隐私同意按钮](https://developers.weixin.qq.com/miniprogram/dev/api/open-api/user-info/wx.getUserProfile.html)与[获取用户信息组件](https://developers.weixin.qq.com/miniprogram/dev/api/open-api/user-info/wx.getUserProfile.html)耦合使用，调用方式为`<button open-type="getUserInfo|agreePrivacyAuthorization">`
...
- `{ "errMsg": "A:fail api scope is not declared in the privacy agreement", "errno": 112 }` 使用到了 A 隐私接口，但是开发者未在「MP后台-设置-服务内容声明-用户隐私保护指引」中声明收集 A 接口对应的隐私类型。补充的隐私类型声明，将在 5 分钟后生效。
...
需要注意的是，用户可能拒绝官方隐私授权弹窗，为了避免过度弹窗打扰用户，开发者再次调用隐私相关接口时，若距上次用户拒绝不足 10 秒，将不再触发弹窗，直接给到开发者用户拒绝隐私授权弹窗的报错。

官方隐私弹窗将有两种样式：

1）与授权弹窗耦合样式：用户在此弹窗下需要勾选隐私协议才可以进行允许操作，若用户在弹窗中拒绝，报错信息为用户拒绝（错误码为 103）。

![](https://res8.wxqcloud.qq.com.cn/wxdoc/e16b8b16-8ed2-459f-9fc4-90410d2fdc88.png)

2）直接弹窗样式：用户侧直接针对隐私协议的授权，若用户在弹窗中拒绝，报错信息为用户未同意隐私协议（错误码为 104）。

![](https://res8.wxqcloud.qq.com.cn/wxdoc/85fc99b8-e789-452c-a9b2-43ea2ac7784f.png)
