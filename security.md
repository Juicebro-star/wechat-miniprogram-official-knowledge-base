...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/security.html"
source_route: "/miniprogram/dev/framework/security.html"
raw_html: "raw-html/security.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/security.html](https://developers.weixin.qq.com/miniprogram/dev/framework/security.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/security.html`
...
1. 1.互不信任原则，不要信任用户提交的数据，包括第三方系统提供的数据，必要的数据校验必须放在后台校验。
2. 2.最小权限原则，代码、模块等只拥有可以完成任务的最小权限，不赋予不必要的权限。
3. 3.禁止明文保存用户敏感数据。
4. 4.小程序代码（不包括云函数代码）跟传统 Web 应用的前端代码类似，可被外部获取及进行反混淆，重要业务逻辑应放在后台代码或云函数中进行。
5. 5.后台接口调用以及云函数调用，必须进行有效的身份鉴权。
...
1. 1.敏感数据、能力相关接口需要在后台进行鉴权。通常可校验 openid、IP 地址、自定义登陆态等信息。
2. 2.鉴权逻辑应放在后台进行，不应在小程序前端以隐藏页面、隐藏按钮等方式来代替。参照原则4。
3. 3.鉴权代码示例（仅供参考）
...
1. 1.当使用 git、 svn 等版本管理工具时，会产生 .git 等目录。某些编辑器或软件也会在运行过程中生成临时文件。若这些目录或文件被带到生产环境，则可能发生源码泄漏。

1. 2.使用[小程序代码管理平台](https://git.weixin.qq.com)或 github 等第三方平台时需要注意项目权限，不要公开敏感、内部项目。

**开发建议：**

1. 1.备份文件和版本管理工具产生的文件不要同步到 Web 目录下。
2. 2.禁止外部访问 .git 等目录与文件。
3. 3.在[小程序代码管理平台](https://git.weixin.qq.com)等管理平台内配置适当的访问权限。
...
1. 1.敏感信息不应以明文、注释、可逆的编码方式（如 base64）、不安全散列函数（如 MD5、SHA1）等形式出现在小程序文件内。
2. 2.部分敏感信息如用户的银行卡号、手机号等需要用于展示的，需要进行脱敏处理。常用脱敏规范如下：
...
1. 3.**如果小程序存在敏感信息泄露的问题，微信开放平台将有可能下架该小程序，并暂停该小程序的相关服务。**

### 授权用户信息变更

1. 1.授权用户资料变更：当部分用户的资料存在风险时，平台会对用户资料进行清理，并通过消息推送服务器通知最近30天授权过的小程序开发者，我们建议开发者留意响应该事件，及时主动更新或清理用户的头像及昵称，降低风险。
2. 2.授权用户资料撤回：当用户撤回授权信息时，平台会通过消息推送服务器通知给小程序开发者，请开发者注意及时删除用户信息。
3. 3.授权用户完成注销：当授权用户完成注销后，平台会通过消息推送服务器通知给小程序开发者，请依法依规及时履行相应个人信息保护义务，保护用户权益。

点击查看[消息推送服务器配置](https://developers.weixin.qq.com/miniprogram/dev/wxcloudservice/wxcloudrun/src/development/weixin/callback)
...
1. 1.1字段【Event】等于"wxa_punish_event"时，则表示这是一条关于【小程序违规处罚事件】的通知。
...
- •当**event_type=1**时**detail**中的JSON字符串结构如下：
...
- •当**event_type=2**时**detail**中的JSON字符串结构如下：
...
- •当**event_type=3**时**detail**中的JSON字符串结构如下：

| 属性 | 类型 | 说明 |
| --- | --- | --- |
| suspended_days | number | 下架天数。注：0代表永久下架 |

- •当**event_type=4**时**detail**中的JSON字符串结构如下：

| 属性 | 类型 | 说明 |
| --- | --- | --- |
| banned_days | number | 账号封禁天数。注：0代表永久封禁 |

- •当**event_type=5**时**detail**中的JSON字符串结构如下：
...
1. 1.使用数据库提供的参数化查询来进行数据库操作，不允许直接通过拼接字符串的方式来合成 SQL 语句。
2. 2.如果存在部分情况需要通过拼接的方式来合成 SQL ，拼接的变量必须要经过处理：
   - •对于整数，需要判断变量是否为整数类型。
   - •对于字符串，需要对单引号、双引号等做转义处理。
3. 3.避免 Web 应用显示 SQL 的报错信息。
4. 4.保证 Web 应用里每一数据层的编码统一。
...
1. 1.后台服务禁用默认账号，修改后台弱口令。
2. 2.敏感服务增加二次验证机制，如短信验证码、邮箱验证码等。
...
1. 1.正确限制可下载文件所在的目录范围
2. 2.通过指定文件 id 的方式来查找下载对应的文件
