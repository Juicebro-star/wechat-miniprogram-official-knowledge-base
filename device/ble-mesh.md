...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/device/ble-mesh.html"
source_route: "/miniprogram/dev/framework/device/ble-mesh.html"
raw_html: "raw-html/device/ble-mesh.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/device/ble-mesh.html](https://developers.weixin.qq.com/miniprogram/dev/framework/device/ble-mesh.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/device/ble-mesh.html`

# 蓝牙低功耗网状网络（BLE Mesh）

蓝牙低功耗网状网络（BLE Mesh）是建立在[**蓝牙低功耗（BLE）**](ble)协议基础上的一种通信协议，它允许大量 BLE 设备组成一个网状网络，在足够大的物理覆盖范围内实现设备之间的互联与协同控制，从而满足多设备场景下的通信需求。
...
在使用插件前，首先需要在小程序管理后台的「设置 - 第三方服务 - 插件管理」中添加插件，开发者可以登录小程序管理后台，通过 appid - wx013447465d3aa024 查找插件并添加。然后在 `app.json` 文件中声明指定版本的插件。关于小程序插件的具体介绍可参考「[使用插件](../plugin/using)」。

目前只有申请了「[工具—设备管理](device-access)」服务类目的小程序才能使用「小程序 BLE Mesh 插件」。
