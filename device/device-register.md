...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/device/device-register.html"
source_route: "/miniprogram/dev/framework/device/device-register.html"
raw_html: "raw-html/device/device-register.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/device/device-register.html](https://developers.weixin.qq.com/miniprogram/dev/framework/device/device-register.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/device/device-register.html`
...
- 设备支持 TEE，并能按照[《设备认证 TEE 规范》](device-tee)开发 TA 并提交验收。
...
**注意：ARM 64 位版本（TEE）需要设备商按照规范开发 TEE 对应的 TA 模块，详细规范与流程参考[设备认证 TEE 规范](device-tee)。**
...
在完成 RPMBD 服务部署后，需要[使用 WMPF 认证设备](device-register-wmpf)。

## 4. 设备认证（Linux）

- 使用「小程序音视频通话 SDK（直连 Linux 设备）」的设备，请使用[wx_device_register](voip/voip-sdk)注册设备。

## 5. 常见问题

**(1) 注册设备报错 `emmc write fail00`**

检查 rpmbd 服务启动参数里的 rpmb 分区路径是否正确。
若路径正确，确认此路径对应的 rpmb 分区在 Android OS 下能否被访问。
...
绝大多数情况是当前设备之前使用不同的 modelId/sn 进行了注册。如使用 WMPF 注册设备，可以使用 [getMiniProgramDeviceInfo](https://developers.weixin.qq.com/doc/oplatform/Miniprogram_Frame/api/cli/miniprogramdevice/getMiniProgramDeviceInfo) 检查下当前设备内的 sn 和 modelId，和传入的是否一致。
