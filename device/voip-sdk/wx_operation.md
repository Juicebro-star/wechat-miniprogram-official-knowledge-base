...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip-sdk/wx_operation.html"
source_route: "/miniprogram/dev/framework/device/voip-sdk/wx_operation.html"
raw_html: "raw-html/device/voip-sdk/wx_operation.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip-sdk/wx_operation.html](https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip-sdk/wx_operation.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/device/voip-sdk/wx_operation.html`
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| operation | wx_operation_t | 想要等待的 operation 对象 |
| timeout_ms | uint32_t | 超时时间，单位毫秒。设置为 `0` 表示不超时。 |
...
| 错误码 | 说明 |
| --- | --- |
| WXERROR_OK | 接口调用正常完成，operation 对象会被自动释放。 |
| WXERROR_TIMEOUT | operation 超时。 |
| 其他 | 接口调用异常返回错误，operation 对象会被自动释放。 |
...
| user_data | void* | 想要在回调函数里接收的自定义数据。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| operation | wx_operation_t | 想要取消的 operation 对象 |
...
主动释放 `wx_operation_t` 对象，但不会取消异步操作本身。调用该函数之后，不应该再对 `wx_operation_t` 对象执行任何操作。
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| operation | wx_operation_t | 想要释放的 operation 对象 |
