...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip/cloud-server-sdk.html"
source_route: "/miniprogram/dev/framework/device/voip/cloud-server-sdk.html"
raw_html: "raw-html/device/voip/cloud-server-sdk.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip/cloud-server-sdk.html](https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip/cloud-server-sdk.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/device/voip/cloud-server-sdk.html`
...
对指定的 openid 发起呼叫后，若呼叫成功，微信开放平台会按照标准的 [消息推送](../../server-ability/message-push) 方法向开发者后台推送消息。
开发者按照[消息推送](../../server-ability/message-push)指引配置回调地址，消息加密方式必须选择「安全模式」。

需要注意的是，服务商角色可能需要服务多个小程序，可以使用 [第三方平台开发](https://developers.weixin.qq.com/doc/oplatform/Third-party_Platforms/2.0/operation/thirdparty/config)。
...
| 参数 | 数据类型 | 说明 |
| --- | --- | --- |
| Action | String | 平台访问开发者后台的事件类型，本接口固定为 `join_voip_room` |
| RoomId | String | 本次通话的房间 id |
| SessionKey | String | 本次通话的 sessionkey |
| ServerToken | String | 本次通话的服务端凭证 |
| Payload | String | 发起通话时第三方传入的自定义 payload |
...
1. [硬件设备接入指引](../device-access)
2. [小程序设备消息](../device-message)
