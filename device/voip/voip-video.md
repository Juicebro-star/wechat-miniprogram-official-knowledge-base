...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip/voip-video.html"
source_route: "/miniprogram/dev/framework/device/voip/voip-video.html"
raw_html: "raw-html/device/voip/voip-video.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip/voip-video.html](https://developers.weixin.qq.com/miniprogram/dev/framework/device/voip/voip-video.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/device/voip/voip-video.html`

# VoIP 视频流指南

小程序 Voip 的视频流默认为 H264 编码流，从设备端看，分为设备上行流与微信下行流，分辨率目前最大为 640x480。

对于上行流，是设备采集 -> 设备编码 -> 网络传输 -> 微信接收 -> 微信解码 -> 小程序渲染。
对于下行流，是微信采集 -> 微信编码 -> 网络传输 -> 设备接收 -> 设备解码 -> 设备渲染。

需要注意的是，微信客户端使用摄像头采集视频流的原始分辨率为 640x480，并且它的方向与物理方向并不一致，所以针对 RTOS 和 Linux 设备，它们收到流后，需要进行后处理才能渲染正确。

![](https://res8.wxqcloud.qq.com.cn/wxdoc/d1484c12-8bd3-435b-8b1f-2d56ad8bf57d.png)
...
- 微信小程序上使用插件的 [setUIConfig](../voip-plugin/api/setUIConfig) 配置**对方**的旋转方向为 270。
...
| 参数 | 取值 | 说明 |
| --- | --- | --- |
| encodeVideoFixedLength | 320 | 240x320 的长边为 320 |
...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/3f1580f3-d346-4286-a940-528b88be4de9.png)
...
- wx_cloudvoip_client_call 接口返回成功后，微信侧即响铃。当然，对于不同的手机，延时情况会不一样。对于 iphone，还依赖于 apple 的消息推送，对于 android，各种系统设置还可能导致手机消息异常，可参考[通话提醒异常排查指南](notification)
...
- 典型的场景，设备采集视频 -> 传送到云端 -> 送入云端 SDK 接口 -> 微信小程序端出图:
  ![](https://res8.wxqcloud.qq.com.cn/wxdoc/0a57c41e-5ba9-4c6c-869f-47223690366f.png)
  整个过程的延时大约在 200 ~ 300ms。
