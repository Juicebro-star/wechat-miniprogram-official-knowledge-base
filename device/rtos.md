...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/device/rtos.html"
source_route: "/miniprogram/dev/framework/device/rtos.html"
raw_html: "raw-html/device/rtos.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/device/rtos.html](https://developers.weixin.qq.com/miniprogram/dev/framework/device/rtos.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/device/rtos.html`
...
可以使用小程序中 Page、Component 能力。支持完整的 WXML、WXSS 语法。
...
- 当使用[组件间通信与事件](../component-framework/event-system#%E8%87%AA%E5%AE%9A%E4%B9%89%E7%9A%84%E7%BB%84%E4%BB%B6%E5%AE%9E%E4%BE%8B%E8%8E%B7%E5%8F%96%E7%BB%93%E6%9E%9C)时，可以直接定义 export 属性，不需要声明 `wx://component-export`。
...
- behavior
- WXS
- 暂不支持[自定义组件](https://developers.weixin.qq.com/miniprogram/dev/reference/api/Component)的下列方法：
...
| 属性 | 类型 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- | --- |
| src | string |  | 否 | 图片资源地址。如果使用代码包内图片资源，src需为绝对路径。 |
...
对于需要展示小程序码的情况，可以参考[扫普通链接二维码打开小程序](https://developers.weixin.qq.com/miniprogram/introduction/qrcode#%E5%8A%9F%E8%83%BD%E4%BB%8B%E7%BB%8D)。

| 属性 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| value | string | 否 | "" | 二维码文本内容 |
| light-color | HexColor | 否 | #000000 | 二维码颜色 |
| dark-color | HexColor | 否 | #FFFFFF | 二维码背景颜色 |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| backgroundColor | HexColor | 否 | #FFFFFF | 窗口的背景色 |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| request | number | 否 | 60_000 | wx.request 的超时时间，单位：毫秒。 |
...
| 属性 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| component | boolean | 否 | 一个自定义组件由 json、wxml、wxss、js 4个文件组成。如设置component为true，将声明这一组文件为自定义组件 |
| styleIsolation | string | 否 | 样式隔离 |
| usingComponents | Object | 否 | 全局自定义组件配置 |
...
### 3.1 注册穿戴设备小程序框架专属 AppID

前置条件：名下需有普通小程序的AppID，并作为该小程序的管理员。  
步骤：
...
![前往控制台](https://res.wx.qq.com/op_res/raa9VJoh96BSei6YXNbcWokn1wWCen3ZqU3PulV8mzzPgX70OkOlljXY0LSiBFd0td91g3HhuIrgwhJWqpibUA)

4. 选择「我的业务--小程序」

![选择小程序](https://res.wx.qq.com/op_res/raa9VJoh96BSei6YXNbcWkBj8IhNtZFNexTvTq42H2-JkyRcf6iAPfhmCsZpJaYXyXDJzkpJ8l52UQ3xupRepA)

5. 页面右上角，切换到穿戴设备小程序要挂靠的普通小程序。
6. 切换到「硬件设置」
7. 点击「申请创建」

![申请创建](https://res.wx.qq.com/op_res/A16GIDL5mvuCu5bJb5xkqWfGBiXIFRceEDDVijk9h1O-SxFIiuYXYfj8_40BQg6-Cck8CO_oAt056O73Ia8zyQ)

8. 创建成功，获得「穿戴端 AppID」

![创建成功](https://res.wx.qq.com/op_res/A16GIDL5mvuCu5bJb5xkqdvnIOps_GPn8C5hUz1jPqBby0mcrNc4cVcj86i0FC9ZvJaeOOKSOgGbPigsiWbSww)
...
![低功耗硬件小程序模式](https://res.wx.qq.com/op_res/A16GIDL5mvuCu5bJb5xkqUysxgyRRaXjTer6Ze6bwtPWtdLYQbTbflrV_N4VBwKFl8sUZ-OaZS7ggtwMhByOwQ)

### 3.3 启用低功耗小程序模式

装好插件后，将开发模式切换到「低功耗硬件小程序模式」

![切换模式](https://res.wx.qq.com/op_res/A16GIDL5mvuCu5bJb5xkqbCCVmdFcBjmaJH3HdBnSCKuc8alKnvDiwqfg3UT53iQKynrCpuf6yjLgz2C8ebqAA)

### 3.4 开发调试

点击「详情」，确保其中设置的AppID与微信开发者平台一致。

![详情](https://res.wx.qq.com/op_res/A16GIDL5mvuCu5bJb5xkqZfWSxp8NuuNWGGg_f-4ycPfEqNfl8yMQinGTtQ0QO1y7tAnqRvHIVMgGOWFJiV2Lw)

我们不建议开发者将JS代码编译为ES5。穿戴设备小程序框架支持ES2023，减少不必要的polyfill有助于显著节省最终小程序代码包体积，进而节省运行时内存占用。

### 3.5 体验与发布

完成开发后，可在「微信开发者工具」中点击「上传」。新上传的版本将覆盖旧的「开发版小程序」。  
开发者可以前往[微信开发者平台](https://developers.weixin.qq.com/platform/)，管理小程序版本、提交审核、发布上线。
