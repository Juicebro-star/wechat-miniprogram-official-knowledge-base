...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/plugin/development.html"
source_route: "/miniprogram/dev/framework/plugin/development.html"
raw_html: "raw-html/plugin/development.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/plugin/development.html](https://developers.weixin.qq.com/miniprogram/dev/framework/plugin/development.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/plugin/development.html`

# 开发插件

开发插件前，请阅读了解 [《小程序插件接入指南》](https://developers.weixin.qq.com/miniprogram/introduction/plugin) 了解开通流程及开放范围，并开通插件功能。如果未开通插件功能，将无法上传插件。

## 创建插件项目

插件类型的项目可以在开发者工具中直接创建。[详情](https://developers.weixin.qq.com/miniprogram/dev/devtools/plugin)

![创建插件](../assets/5735d92d37f3232d.png)
...
请注意：在插件开发中，只有 [部分接口](api-limit) 可以直接调用；另外还有部分能力（如 获取用户信息 和 发起支付 等）可以通过 [插件功能页](functional-pages) 的方式使用。

### 自定义组件

插件可以定义若干个自定义组件，这些自定义组件都可以在插件内相互引用。但提供给使用者小程序使用的自定义组件必须在配置文件的 `publicComponents` 段中列出（参考上文）。

除去接口限制以外，自定义组件的编写和组织方式与一般的自定义组件相同，每个自定义组件由 `wxml`, `wxss`, `js` 和 `json` 四个文件组成。具体可以参考 [自定义组件的文档](../component-framework/index)。

### 页面

插件从小程序基础库版本 [2.1.0](../compatibility) 开始支持页面。插件可以定义若干个插件页面，可以从本插件的自定义组件、其他页面中跳转，或从使用者小程序中跳转。所有页面必须在配置文件的 `pages` 段中列出（参考上文）。
...
自基础库版本 [2.2.2](../compatibility) 开始，在插件自身的页面中，插件还可以调用 [wx.navigateTo](https://developers.weixin.qq.com/miniprogram/dev/api/navigate/wx.navigateToMiniProgram.html) 来进行页面跳转， `url` 格式与使用 `navigator` 组件时相仿。
...
> [在开发者工具中预览效果](https://developers.weixin.qq.com/s/GbXmMLml7vjC "在开发者工具中预览效果")，需要手动填写一下 `miniprogram/app.json` 中的插件 AppID

从基础库 [2.11.1](../compatibility) 起，在插件中有全局函数 `requireMiniProgram`，可以获取由使用者小程序导出的内容。
...
另外也可以 [参考使用者小程序的相关文档](using#导出到插件)

### 引用小程序的自定义组件

> [在开发者工具中预览效果](https://developers.weixin.qq.com/s/QRRovLmu7Xjm "在开发者工具中预览效果")，需要手动填写一下 `miniprogram/app.json` 中的插件 AppID

有时，插件可能需要在页面或者自定义组件中，将一部分区域交给使用的小程序来渲染，因此需要能够引用小程序的自定义组件。但由于插件中不能直接指定小程序的自定义组件路径，因此无法直接通过 `usingComponents` 的方式来引用。这里介绍通过 [抽象节点（generics）](../custom-component/generics) 来实现引用的方式。
...
如果是插件页，插件页本身就是一个页面顶层组件，小程序不会引用它，无法通过 `generic:xxx=""` 的方式来指定抽象节点实现；因此，从基础库 [2.12.2](../compatibility) 起，小程序可以在插件的配置里为插件页指定抽象节点实现。例如插件页面名为 `plugin-index`，则可以：
...
另外也可以 [参考使用者小程序的相关文档](using#为插件提供自定义组件)
...
- （建议的方式）如果当前开发者有 [测试号](https://developers.weixin.qq.com/miniprogram/dev/devtools/sandbox)，则会使用这个测试号；在测试号的设置页中可以看到测试号的 `appid` 、 `appsecret` 并设置域名列表。
...
自基础库版本 [2.0.7](../compatibility) 开始，在小程序运行期间，若网络状况正常， `NONCESTR` 和 `TIMESTAMP` 会每 10 分钟变更一次。如有必要，可以通过判断 `TIMESTAMP` 来确定当前签名是否依旧有效。
