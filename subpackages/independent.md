...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/subpackages/independent.html"
source_route: "/miniprogram/dev/framework/subpackages/independent.html"
raw_html: "raw-html/subpackages/independent.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/subpackages/independent.html](https://developers.weixin.qq.com/miniprogram/dev/framework/subpackages/independent.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/subpackages/independent.html`

# 独立分包

> 微信客户端 6.7.2，基础库 [2.3.0](../compatibility) 及以上版本开始支持。开发者工具请使用 1.02.1808300 及以上版本，可 [点此下载](https://developers.weixin.qq.com/miniprogram/dev/devtools/download)。
...
> 小游戏在基础库 v2.12.2 开始支持独立分包，详见 [小游戏独立分包指南](https://developers.weixin.qq.com/minigame/dev/guide/base-ability/independent-sub-packages)。
...
- **独立分包中不能依赖主包和其他分包中的内容**，包括 JS 文件、template、wxss、自定义组件、插件等（使用 [分包异步化](async) 时 JS 文件、自定义组件、插件不受此条限制）
...
为了在独立分包中满足这一需求，基础库 [2.2.4](../compatibility) 版本开始 `getApp` 支持 `allowDefault` 参数，在 `App` 未定义时返回一个默认实现。当主包加载，`App` 被注册时，默认实现中定义的属性会被覆盖合并到真正的 `App` 中。
