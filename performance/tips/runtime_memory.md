...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/performance/tips/runtime_memory.html"
source_route: "/miniprogram/dev/framework/performance/tips/runtime_memory.html"
raw_html: "raw-html/performance/tips/runtime_memory.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/performance/tips/runtime_memory.html](https://developers.weixin.qq.com/miniprogram/dev/framework/performance/tips/runtime_memory.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/performance/tips/runtime_memory.html`

# 内存优化

## 1. 合理使用分包加载

使用[分包加载](../../subpackages/basic)不仅能优化启动耗时，也能够实现页面、组件和逻辑较粗粒度的按需加载，从而降低内存的占用。详情请参考[《启动优化-代码包体积优化》](start_optimizeA#_1-%E5%90%88%E7%90%86%E4%BD%BF%E7%94%A8%E5%88%86%E5%8C%85%E5%8A%A0%E8%BD%BD)

## 2. 使用[按需注入](../../ability/lazyload#按需注入)和[用时注入](../../ability/lazyload#用时注入)

通过开启「按需注入」和「用时注入」，可以在运行时避免加载未使用到的页面和组件，降低运行时的内存占用。详情请参考[《启动优化-代码注入优化》](start_optimizeB)

## 3. 内存分析

如果要更精细的分析小程序逻辑层的内存分布情况，可以使用开发者工具调试器的「[内存调试](../devtools-perf#内存调试)」或「真机调试 2.0」提供的「[内存调试](../remote_debug_2#内存调试)」能力。
...
存在内存泄露问题会导致小程序在运行过程中内存占用持续增长，引起小程序闪退或被微信强制销毁。
