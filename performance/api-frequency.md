...
official_url: "https://developers.weixin.qq.com/miniprogram/dev/framework/performance/api-frequency.html"
source_route: "/miniprogram/dev/framework/performance/api-frequency.html"
raw_html: "raw-html/performance/api-frequency.html"
---

> 来源：微信开放文档（官方页面）
> 原始链接：[https://developers.weixin.qq.com/miniprogram/dev/framework/performance/api-frequency.html](https://developers.weixin.qq.com/miniprogram/dev/framework/performance/api-frequency.html)
> 抓取日期：2026-09-05
> 原始 HTML：`raw-html/performance/api-frequency.html`
...
「限频接口」指的是一个用户在一段时间内不允许频繁调用的 wx 接口，此类接口一般会调用到微信后台系统资源，为了保护系统，同时防止用户资源被滥用，开发者需要对此类接口做适度的频率限制，不能无节制地调用。

平台会对小程序内「限频接口」的调用情况做监控，如果小程序对此类接口的调用频率超出平台的规范，将会收到站内信提醒。系统会在资源紧张的情况下优先保障合理使用的小程序的服务。

开发者可登录「小程序管理后台 - 开发管理 - 接口设置」中查看「限频接口」调用情况。
...
| API | 规范 | 其他说明 |
| --- | --- | --- |
| wx.login | 一天的调用总次数不多于该小程序 PV 的两倍,单用户一秒钟不能大于4次 | - |
| wx.checkSession | 一天的调用总次数不多于该小程序 PV 的两倍,单用户一秒钟不能大于4次 | - |
| wx.getSetting | 一天的调用总次数不多于该小程序 PV 的两倍,单用户一秒钟不能大于4次 | - |
| wx.getUserInfo | 一天的调用总次数不多于该小程序 PV 的两倍,单用户一秒钟不能大于4次 | - |
| wx.getUserProfile | 一天的调用总次数不多于该小程序 PV 的两倍,单用户一秒钟不能大于4次 | - |
